"""
FlashInfer-Bench Modal Baseline Benchmark Runner (gdn_decode).

Benchmarks the FlashInfer baseline solution (`flashinfer_wrapper_9b7f1e`)
...
Setup (reuses the same volume as run_modal.py):
    modal setup
    modal volume create flashinfer-trace        # already created
    # dataset already uploaded

Smoke test first (1 workload only) to verify that flashinfer-python
installs on Modal and that the baseline builds on B200:
...
app = modal.App("flashinfer-bench-baseline-gdn-decode")
...
BASELINE_SOLUTION_NAME = "flashinfer_wrapper_9b7f1e"

# Baseline needs `flashinfer-python` (note: PyPI name, not `flashinfer`).
# flashinfer-cubin ships pre-compiled kernels incl. Blackwell (SM100) so we
# skip JIT compilation on first run.
image = (
...
# Baseline targets H20/H100/H200 per its solution JSON. B200 is worth a try
# because our own kernel runs on B200 — matching hardware avoids cross-arch
# latency drift when scoring. Fall back to H200/H100 if baseline doesn't
# compile/run on B200.
...
def run_baseline(smoke: bool = False) -> dict:
    ...
    if smoke:
        # Minimal config + workload budget for smoke test
        config = BenchmarkConfig(warmup_runs=1, iterations=5, num_trials=1)
    else:
        # Match run_modal.py's config so latency numbers are comparable
    ...
    # Persist traces to the Modal volume (matches run_modal.py).
    trace_volume.commit()
