...
@triton.autotune(
    configs=[
        ...
        triton.Config({'BLOCK_V': 128}, num_warps=8, num_stages=2),
...
def gdn_decode_d128_kernel(
    ...
    """
    Specialized Gated Delta Net decoding kernel for qk4 / v8 / d128.

    Each program handles one (batch, v_head, v_tile, seq) combination and
    always processes the full K dimension in a single pass.
    """
    pid_batch = tl.program_id(0)
    pid_v_head = tl.program_id(1)
    pid_compound = tl.program_id(2)

    num_v_tiles = 128 // BLOCK_V
    pid_v_tile = pid_compound % num_v_tiles
    pid_seq = pid_compound // num_v_tiles

    pid_qk_head = pid_v_head // 2
    ...
    q_chunk = tl.load(q_base + k_off * stride_q_d).to(tl.float32)
    k_chunk = tl.load(k_base + k_off * stride_k_d).to(tl.float32)
    state_ptrs = (state_base
                 + v_off[:, None] * stride_state_v
                 + k_off[None, :] * stride_state_k)
    state_chunk = tl.load(state_ptrs, mask=v_mask[:, None], other=0.0).to(tl.float32)
    ...
    tl.store(ns_ptrs, new_state_chunk.to(tl.float32), mask=v_mask[:, None])
...
def gdn_decode(
    ...
    assert head_size == 128, "tera_v3 gdn_decode is specialized for head_size=128"
    assert num_q_heads == 4, "tera_v3 gdn_decode is specialized for num_q_heads=4"
    assert k.shape[2] == 4, "tera_v3 gdn_decode is specialized for num_k_heads=4"
    assert num_v_heads == 8, "tera_v3 gdn_decode is specialized for num_v_heads=8"
    ...
    def grid(META):
        num_v_tiles = 128 // META['BLOCK_V']
        return (batch_size, num_v_heads, num_v_tiles * seq_len)

    gdn_decode_d128_kernel[grid](
        q, k, v,
        state,
        A_log, a, dt_bias, b,
        output, new_state,
